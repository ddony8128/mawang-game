...
import type { CooldownItem, GameLogState, LogItem } from "../types/effects";
import type { RoleKey, SkillKey } from "../types/identity";

type PrivateUpdate = NonNullable<EngineDelta["privateByPlayer"]>[string];

function ensurePrivateUpdate(
  map: Map<string, PrivateUpdate>,
  playerId: string,
): PrivateUpdate {
...
export class GameEngineImpl implements GameEngine {
  ...
  constructor(timerRegistry?: TimerRegistry) {
    this.timerRegistry = timerRegistry;
  }

  // 추후 RoomRuntime 가 초기 GameSnapshot 을 로딩한 뒤 주입해 사용한다.
  setState(snapshot: GameSnapshot): void {
    this.state = snapshot;
  }
  ...
  private getNowMs(): number {
    if (this.timerRegistry) return this.timerRegistry.nowMs();
    if (this.state) return this.state.timers.nowMs;
    return Date.now();
  }
  ...
  processLoop(): EngineOutput | null {
    ...
    const appliedAcks: NonNullable<EngineOutput["appliedAcks"]> = [];
    const invalidActions: NonNullable<EngineOutput["invalidActions"]> = [];

    // 간단한 delta: HP/생존 정보 + 개인 상태 변경 신호만 포함
    const publicPlayerUpdates = new Map<
      string,
      { playerId: string; hp: number; alive: boolean }
    >();
    const privateUpdates = new Map<string, PrivateUpdate>();
    ...
    let delta: EngineDelta | undefined;
    if (deltaPlayers || privateUpdates.size > 0) {
      const publicDelta: EngineDelta["public"] = {};
      if (deltaPlayers) {
  ...
  private handlePlayerAction(
    task: Extract<EngineTask, { kind: "PLAYER_ACTION" }>,
    ctx: {
      appliedAcks: NonNullable<EngineOutput["appliedAcks"]>;
      invalidActions: NonNullable<EngineOutput["invalidActions"]>;
      publicPlayerUpdates: Map<
        string,
        { playerId: string; hp: number; alive: boolean }
      >;
      privateUpdates: Map<string, PrivateUpdate>;
    },
  ) {
    ...
    const feared = actor.effects.find(
      (e: any) => e.kind === "feared" && e.payload?.untilMs > nowMs,
    );
    if (feared) {
      ctx.invalidActions.push({
        actionId,
    ...
    if (
      !ctx.invalidActions.some((ia) => ia.actionId === actionId) &&
  ...
  private handleCardUse(
    ...
    const cardTypeFromPayload: string | undefined = data?.cardType;
    const cardInstanceIdsRaw: unknown = data?.cardInstanceIds;
    const primaryInstanceId: string | undefined = data?.cardInstanceId;
    const targetPlayerId: string | null | undefined = data?.targetPlayerId ?? null;

    let cardInstanceIds: string[] = [];
    if (Array.isArray(cardInstanceIdsRaw)) {
      cardInstanceIds = cardInstanceIdsRaw.filter(
    ...
    const isCoward =
      this.state.players[actor.identity.playerId]?.role === "coward";
    if (isCoward) {
      ...
      if (r < 0.5) {
        ...
        if (!update.logItems) update.logItems = [];
    ...
    if (card.type === "knife") {
      ...
      const targetUpdate = ensurePrivateUpdate(
        ctx.privateUpdates,
        target.identity.playerId,
      );
      ...
      const useMode: string | undefined = data?.useMode;
      const requiredCount =
        useMode === "magnifier2" ? 2 : useMode === "magnifier3" ? 3 : 0;
  ...
  private handleDiscard(
    ...
    const ids: unknown = data?.cardInstanceIds;
    ...
    const idSet = new Set<string>(stringIds);
    actor.hand = actor.hand.filter((c) => !idSet.has(c.id));
    markStateChanged(ctx.privateUpdates, actor);
  }

  private handleCardGive(
    actionId: string,
    actor: PlayerState,
    data: any,
    ctx: {
    ...
    const instanceId: string | undefined = data?.cardInstanceId;
    const toPlayerId: string | undefined = data?.toPlayerId;
    ...
    const receiverUpdate = ensurePrivateUpdate(
      ctx.privateUpdates,
      target.identity.playerId,
    );
  ...
  private handleAbility(
    ...
    const skillKey: string | undefined = data?.skillKey;
    const targetPlayerId: string | null | undefined = data?.targetPlayerId ?? null;
    ...
    const requireRole = (role: RoleKey) => {
      if (actor.role !== role) {
        ctx.invalidActions.push({
          actionId,
          playerId: actor.identity.playerId,
    ...
    const requireCooldown = (key: SkillKey, cooldownMs: number) => {
    ...
    if (skillKey === "healer_heal") {
      if (!requireRole("healer")) return;
      if (!requireCooldown("healer_heal", 180_000)) return; // 3분
    ...
    if (skillKey === "parry_shield") {
      if (!requireRole("parryman")) return;
      if (!requireCooldown("parry_shield", 180_000)) return;
    ...
    if (skillKey === "mawang_fear") {
      if (!requireRole("mawang_fear")) return;
      if (!requireCooldown("mawang_fear", 180_000)) return;
    ...
    if (skillKey === "mawang_mask") {
      if (!requireRole("mawang_troll")) return;
      ...
      const fakeRole = data?.fakeRole as RoleKey | undefined;
      const fakeTeam = data?.fakeTeam as PlayerState["team"] | undefined;
      const fakeSide = data?.fakeSide as PlayerState["side"] | undefined;
      ...
      if (fakeTeam) actor.fake.fakeTeam = fakeTeam;
      if (fakeSide) actor.fake.fakeSide = fakeSide;
    ...
    if (skillKey === "traitor_beer") {
      if (!requireRole("fallen")) return;
      if (!requireCooldown("traitor_beer", 180_000)) return;
    ...
    if (skillKey === "slayer_ult") {
      if (!requireRole("slayer")) return;

      const used = actor.effects.some(
        (e: any) => e.kind === "slayerUltUsed",
      );
      if (used) {
        ctx.invalidActions.push({
          actionId,
  ...
  private handleInternalTask(
    task: Extract<EngineTask, { kind: "INTERNAL_TASK" }>,
    ctx: {
      publicPlayerUpdates: Map<
        string,
        { playerId: string; hp: number; alive: boolean }
    ...
    if (task.type === "APPLY_DAMAGE") {
      const playerId: string | undefined = task.payload?.playerId;
      const amount: number | undefined = task.payload?.amount;
      if (!playerId || typeof amount !== "number" || amount <= 0) return;

      const player = this.state.players[playerId];
      if (!player || !player.alive) return;

      // shield 효과가 있으면 피해 무효 처리
      const hasShield = player.effects.some(
        (e: any) => e.kind === "shield" && e.payload?.untilMs > now,
      );
      if (hasShield) {
        return;
      }
      ...
      const playerId: string = task.payload?.playerId;
      const player = this.state.players[playerId];
      if (!player) return;
      ...
      if (player.role === "mawang_fear") {
        const used = player.effects.some(
          (e: any) => e.kind === "fearReviveUsed",
        );
        if (!used) {
          // 공포의 재림: 한 번 부활
        ...
        const stubborn = player.effects.find(
          (e: any) => e.kind === "trollStubborn",
        );
        if (!stubborn) {
  ...
  private handleSystemTask(
    task: Extract<EngineTask, { kind: "SYSTEM_TASK" }>,
    ctx: {
      publicPlayerUpdates: Map<string, { playerId: string; hp: number; alive: boolean }>;
    },
  ) {
    if (!this.state) return;
  ...
  private abortGame() {
    if (!this.state) return;
    if (this.state.meta.state === "ended") return;
  ...
  private handleTimerFired(
    task: Extract<EngineTask, { kind: "TIMER_FIRED" }>,
    ctx: {
      publicPlayerUpdates: Map<
        string,
        { playerId: string; hp: number; alive: boolean }
    ...
    if (task.timerType === "CARD_DRAW") {
      ...
      for (const player of Object.values(this.state.players)) {
        ...
        const beerIdx = player.effects.findIndex(
          (e: any) => e.kind === "beerGuaranteed",
        );
        let firstCardType: CardType;
        if (beerIdx >= 0) {
          firstCardType = "beer";
      ...
      const playerId: string | undefined = task.payload?.playerId;
      const bombId: string | undefined = task.payload?.bombId;
      if (!playerId || !bombId) return;

      const player = this.state.players[playerId];
      if (!player || !player.alive) return;

      const idx = player.effects.findIndex(
        (e: any) => e.kind === "bomb" && e.id === bombId,
      );
      if (idx === -1) return;

      const effect: any = player.effects[idx];
      player.effects.splice(idx, 1);
      markStateChanged(ctx.privateUpdates, player);

      const damage: number = effect.payload?.damage ?? 2;
      this.enqueueInternalTask("APPLY_DAMAGE", {
        playerId,
        amount: damage,
      ...
      const playerId: string | undefined = task.payload?.playerId;
      const effectKind: string | undefined = task.payload?.effectKind;
      if (!playerId || !effectKind) return;

      const player = this.state.players[playerId];
      if (!player) return;

      const idx = player.effects.findIndex(
        (e: any) => e.kind === effectKind,
      );
      if (idx === -1) return;

      const eff: any = player.effects[idx];
      player.effects.splice(idx, 1);
      markStateChanged(ctx.privateUpdates, player);
      ...
      const player = this.state.players[playerId];
      if (!player || player.role !== "weakling") return;
      ...
      let currentFake =
        typeof player.fake.fakeHp === "number" ? player.fake.fakeHp : 3;
      ...
      if (nextFake > 3) nextFake = 3;
      if (nextFake <= 0) nextFake = 1;
  ...
  private checkEndCondition() {
    ...
    const players = Object.values(this.state.players);
    const mawangAlive = players.some(
      (p) => p.alive && this.isMawangRole(p.role),
    );
  ...
  private isMawangRole(role: RoleKey): boolean {
    return role === "mawang_fear" || role === "mawang_troll";
  }

  private isMawangSideWinner(p: PlayerState): boolean {
    // 마왕 + 배신자 승리: evil 팀 또는 traitor side 를 evil 승리로 처리
    return p.team === "evil" || p.side === "traitor";
  }

  private enqueueInternalTask(
    type: Extract<EngineTask, { kind: "INTERNAL_TASK" }>["type"],
    payload: any,
  ) {
    this.enqueue({
      kind: "INTERNAL_TASK",
  ...
  private createCardInstance(type: CardType, now: number): CardInstance {
    return {
      id: crypto.randomUUID(),
      type,
      createdAtMs: now,
  ...
  private drawCardsForPlayer(player: PlayerState, count: number) {
    const now = this.getNowMs();
  ...
  private findCooldown(player: PlayerState, skillKey: SkillKey): CooldownItem | undefined {
    return player.cooldowns.find((c) => c.skillKey === skillKey);
  }

  private setCooldown(player: PlayerState, skillKey: SkillKey, readyAtMs: number) {
