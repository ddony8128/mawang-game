...
exports.createInitialSnapshotForRoom = createInitialSnapshotForRoom;
...
function randomShuffle(arr, rnd) {
    const a = [...arr];
...
function createCard(type, nowMs) {
    return {
...
function roleComposition(count) {
    switch (count) {
        case 6:
...
async function createInitialSnapshotForRoom(roomId) {
    // 최신 running 게임 row를 가져온다.
    const { data: gameRow, error } = await supabase_1.supabase
        .from("games")
        .select("*")
        .eq("room_id", roomId)
        .eq("state", "running")
    ...
    const roomPlayers = await (0, roomPlayersRepo_1.listRoomPlayers)(roomId);
    ...
    const heroRoles = ["parryman", "slayer", "sage", "healer"];
    const civilRoles = [
        "weakling",
        "coward",
        "madman",
    ...
    const chosenCivils = randomShuffle(civilRoles, rng.next).slice(0, civilTarget);
    const traitorRoles = ["aide", "fallen"];
    const chosenTraitors = randomShuffle(traitorRoles, rng.next).slice(0, traitorTarget);
    // 플레이어별 역할 할당 순서:
    ...
    const rest = shuffledPlayers.filter((id) => id !== mawangId && !roleByPlayer.has(id));
    let idx = 0;
    // 1) 배신자 역할 배정
    ...
    for (let i = 0; i < remainingCivilCount; i += 1) {
        ...
        const role = i < civilRolePool.length
            ? civilRolePool[i]
    ...
    const heroRolesPresent = new Set();
    const civilRolesPresent = new Set();
    ...
    for (const p of Object.values(players)) {
        if (p.role === "madman") {
            const fakeHero = heroRoleArray.length > 0
                ? randomChoice(heroRoleArray, rng.next)
                : randomChoice(["parryman", "slayer", "sage", "healer"], rng.next);
